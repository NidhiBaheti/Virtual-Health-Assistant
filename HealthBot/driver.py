...
class ChatInterface(Frame):
    ...
    def msg(self):
        tkinter.messagebox.showinfo("HealthBot v1.0",
                                    'HealthBot is a chatbot for answering health related queries\nIt is based on retrival-based NLP using pythons NLTK tool-kit module\nGUI is based on Tkinter\nIt can answer questions regarding users health status')

    def about(self):
        tkinter.messagebox.showinfo("HealthBot Developers",
                                    "1.Jyot Buch\n2.Aniket Wagh\n3.Nidhi Baheti\n4.Kaushal Oza\n5.Jitesh Tejnani")
    ...
    def run_cnn(self, image):
        # pr = "HealthBot : You have dick disease."
        class_name = ['Acne', 'Eczema', 'Hair Loss', 'Melanoma', 'Nail Fungus']
        """
        Enter the model here, run it and get prediction...
        """
        image = self.preprocess_image(image)
        results = cnn_model.predict(image)
        pr = class_name[np.argmax(results)]
        return pr
    ...
    def send_message_insert(self, message):
        ...
        """
        If image is entered, then predict the skin disease
        """
        if user_input[-3:] == 'jpg' or user_input[-3:] == 'png':
            ob = self.run_cnn(user_input)
            ob = "You might have " + ob +". Please consult a doctor for further queries"
        # """
        # If text is entered , process the text to get output
        # """
        else:
            ob = self.chat(user_input)
        """
        If returned prediction is a skin disease, then ask user to enter an image
        """
        if ob == 'Skin disease symptoms':
            ob = "Please take a picture and enter it in the chat window (jpg or png format)."
